...
from alt.preproc import run_preproc_named_process
from alt.infer import run_infer_named
from alt.util import default_log_config

logger = logging.getLogger(__name__)


def prepare(cpu: bool = False, force: bool = False):
    demucs_model_names = [
        "mdx",
        "mdx_extra",
    ]
    ...
    for demucs_model in demucs_model_names:
        preproc = PreprocConfig(separate=True, demucs_model_name=demucs_model)
        run_preproc_named_process(
            cfg=preproc,
            name=f"01-preproc-{demucs_model}",
            extract_name="01-extract",
            force=force,
            cached=True,
            cpu=cpu,
        )
